...
public class Frogger extends WindowController{
	...
	frog = new Frog(getImage("froggy.gif"), new Location((HIGHWAY_RIGHT-FROG_WIDTH)/2, HIGHWAY_BOTTOM + (LANE_WIDTH-48)/2), (int) LANE_WIDTH, canvas);
	Image v = getImage("jeep_right.gif");

	//Vehicle ve = new Vehicle(v, new Location(0,0), 0.033, HIGHWAY_LENGTH, frog, canvas);
	for(int j = 0; j < 4; j++) {
    	Image vehicle = getImage("jeep_right.gif");
    	double velocity = 0.033 + (0.133 - 0.033) * Math.random(); // Random speed for each lane
    	new Lane(velocity, vehicle, new Location(HIGHWAY_TOP + LANE_WIDTH * j - 20, 0), HIGHWAY_LENGTH, frog, canvas, velocity);
    }
}
